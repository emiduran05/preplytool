...
export default function DashboardStudent() {
  ...
  const [tituloClase, setTituloClase] = useState("");
  const [lecturaHTML, setLecturaHTML] = useState("");
  const [ejerciciosHTML, setEjerciciosHTML] = useState("");
  const [pdfUrl, setPdfUrl] = useState(null);
  ...
  useEffect(() => {
    ...
    fetch(`https://preplytool-2tgl.vercel.app/api/services/leccion/${leccionId}`)
      ...
      .then(data => {
        ...
        // ✅ CONVERSIÓN DELTA → HTML (CAMBIO CLAVE)
        if (data.contenido_leccion?.ops) {
          const converter = new QuillDeltaToHtmlConverter(
            data.contenido_leccion.ops,
            {}
          );
          setLecturaHTML(converter.convert());
        } else {
          setLecturaHTML("");
        }

        setEjerciciosHTML(
          data.ejercicios_leccion
            ? renderEjerciciosConInputs(data.ejercicios_leccion)
            : ""
        );
      });
  ...
  const toggleCompleted = async (checked) => {
    ...
    setCompletedLessons(prev => ({
      ...prev,
      [leccionId]: { ...prev[leccionId], completed: checked }
    }));
  };
        ...
        <div className="dashboard_content">
          {!leccionId ? (
            ...
            <>
              ...
              {lecturaHTML && (
                <ReactQuill value={lecturaHTML} readOnly theme="bubble" />
              )}

              {pdfUrl && (
                <>
                  <iframe src={pdfUrl} width="100%" height="450px" />
                  <a href={pdfUrl} target="_blank">Ver documento completo</a>
                </>
              )}
              ...
              <h3>📝 Notas para la siguiente clase</h3>
              <textarea
                value={currentNotes}
                onChange={e => setCurrentNotes(e.target.value)}
                onBlur={() => saveNotes(leccionId, currentNotes)}
